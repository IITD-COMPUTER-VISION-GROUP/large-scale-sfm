...
int main(int argc, char *argv[])
{
	...
	for ( int cIndex = StIndexG; cIndex < clusterMergeList.size(); cIndex ++)
	{
		...
		double reProjErr[10] = {0.0};
		double distErr[10] = {0.0};
		double distError[matchIndexList.size()][10];

		vector<string> rtsFileList;

		rtsFileList.push_back("RTS2D2D.txt");
		rtsFileList.push_back("RTS3D3D.txt");
		rtsFileList.push_back("RTS2DR3DT.txt");
		rtsFileList.push_back("RTSRefined.txt");
		rtsFileList.push_back("RTSRefined_1.txt");

		for ( int indexT = 0; indexT < rtsFileList.size(); indexT ++ )
		{
			double scale = 1.0; 
			TransformType RT;
		
			string debugFile = outDir + "/dendogram/cluster" + clusterMergeList[cIndex].newCluster + "/debug_" + to_string(indexT) + ".txt";
			FILE *fpDebug = fopen(debugFile.c_str(), "w");

			string RTSFile = outDir + "/dendogram/cluster" + clusterMergeList[cIndex].newCluster + "/" + rtsFileList[indexT];
			ifstream myfile1;
			...
			vector< Camera *> 		cameraListTmp;
			vector< PointAttr *> 		ptAttrListTmp;
			vector< vector<Point2D> *> 	trackListTmp;
			...
			for ( int index2 = 0; index2 < ptAttrList2.size(); index2 ++ )
			{
				...
				pt2 = scale * pt2;
				pt2 = RT.first * pt2 + RT.second;
		
				PointAttr * newPtAttr = new PointAttr;
				newPtAttr->X = pt2[0];
				newPtAttr->Y = pt2[1];
				newPtAttr->Z = pt2[2];
				newPtAttr->pointId = ptAttrList2[index2]->pointId;

				ptAttrListTmp.push_back(newPtAttr);
			}	
			...
			for ( int index2 = 0; index2 < cameraList2.size(); index2 ++ )
			{
				...
				newCam->rotMat[0] = R(0, 0);
				newCam->rotMat[1] = R(0, 1);
				newCam->rotMat[2] = R(0, 2);
				newCam->rotMat[3] = R(1, 0);
				newCam->rotMat[4] = R(1, 1);
				newCam->rotMat[5] = R(1, 2);
				newCam->rotMat[6] = R(2, 0);
				newCam->rotMat[7] = R(2, 1);
				newCam->rotMat[8] = R(2, 2);
				...
				newCam->cMat[1] = pt2[1];
				newCam->cMat[2] = pt2[2];
				newCam->focal = cameraList2[index2]->focal;
				newCam->k1 = cameraList2[index2]->k1;
				newCam->k2 = cameraList2[index2]->k2;

				getTfromRC(newCam->rotMat, newCam->cMat, newCam->tMat);
		
				cameraListTmp.push_back(newCam);

				/*cout << endl << "CAM " << index2 << endl << R << endl;
				cout << pt2 << endl;*/
			}	

			int projCount = 0;
			vector<double> tmpErr, tmpDist;

			for ( int index1 = 0; index1 < matchIndexList.size(); index1 ++ )
			{
				vector<Point2D> *tmpVec = trackList2[matchIndexList[index1].second];

				Eigen::Vector3d pt1 = firstList[index1];
				Eigen::Vector3d pt2 = secondList[index1];

				pt2 = scale * pt2;
				pt2 = RT.first * pt2 + RT.second;

				Eigen::Vector3d pt3 = pt2 -pt1;
				tmpDist.push_back(pt3.norm());

				distError[index1][indexT] = pt3.norm();

				fprintf(fpDebug, "\n[%lf %lf %lf] [%lf %lf %lf] %lf\n", pt1[0], pt1[1], pt1[2], pt2[0], pt2[1], pt2[2], pt3.norm());

				for ( int index2 = 0; index2 < tmpVec->size(); index2 ++ )
				{
					double x = (*tmpVec)[index2].x;
					double y = (*tmpVec)[index2].y;
					int camId = (*tmpVec)[index2].camId;

					Eigen::Vector3d pt;
					pt[0] = ptAttrList1[matchIndexList[index1].first]->X;
					pt[1] = ptAttrList1[matchIndexList[index1].first]->Y;
					pt[2] = ptAttrList1[matchIndexList[index1].first]->Z;

					Eigen::Matrix3d R;
					R << cameraListTmp[camId]->rotMat[0], cameraListTmp[camId]->rotMat[1], cameraListTmp[camId]->rotMat[2],
					     cameraListTmp[camId]->rotMat[3], cameraListTmp[camId]->rotMat[4], cameraListTmp[camId]->rotMat[5],
					     cameraListTmp[camId]->rotMat[6], cameraListTmp[camId]->rotMat[7], cameraListTmp[camId]->rotMat[8];

					Eigen::Vector3d C;
					C[0] = cameraListTmp[camId]->cMat[0];
					C[1] = cameraListTmp[camId]->cMat[1];
					C[2] = cameraListTmp[camId]->cMat[2];

					Eigen::Vector3d X3d = R * (pt - C);

					double mx = X3d[0]/X3d[2];
					double my = X3d[1]/X3d[2];
					double r2  = 1 + cameraListTmp[camId]->k1 * (mx * mx + my * my);
					double umx = r2 * mx * cameraListTmp[camId]->focal;
					double umy = r2 * my * cameraListTmp[camId]->focal;

					double reprojE = sqrt((x - umx)*(x - umx) + (y - umy)*(y - umy));

					fprintf(fpDebug, "[%lf %lf] [%lf %lf] %lf\n", x, y, umx, umy, reprojE);

					tmpErr.push_back(reprojE);

					reProjErr[indexT] += reprojE;
					projCount ++;

					/*cout << camId << "  " << ptAttrListTmp[index2]->pointId << endl;
					cout << R << endl;
					cout << pt << endl;
					cout << C << endl;
					getchar();*/
			...
			double sum_dist= 0;
			for(vector<double>::iterator it = tmpDist.begin(); it != tmpDist.end(); ++it)
			{
			    sum_dist += *it;
			}
			...
			for(vector<double>::iterator it = tmpDist.begin(); it != tmpDist.end(); ++it)
			{
			    if ( *it < sum_dist/(double)tmpDist.size() ) sum_inliers ++;
			}

			printf("###### %d %lf %lf %lf %lf %d\n", indexT, reProjErr[indexT]/projCount, tmpErr[tmpErr.size()/2], tmpDist[tmpDist.size()/2], sum_dist/(double)tmpDist.size(), sum_inliers );

			fprintf(fpDebug, "ReprojErr Mean %lf Median %lf\n", reProjErr[indexT]/projCount, tmpErr[tmpErr.size()/2]);

			fclose(fpDebug);

			reProjErr[indexT] /= projCount;
			distErr[indexT] = sum_dist/(double)tmpDist.size();
		}

		string debugFile = outDir + "/dendogram/cluster" + clusterMergeList[cIndex].newCluster + "/debug_err.txt";
		FILE *fpDebug = fopen(debugFile.c_str(), "w");
		for ( int index1 = 0; index1 < matchIndexList.size(); index1 ++ )
		{
			for ( int indexT = 0; indexT < 4; indexT ++ )
			{
				fprintf(fpDebug, "%lf ", distError[index1][indexT]);
			}
			fprintf(fpDebug, "\n");
		}
		fclose(fpDebug);
		...
		int minIndex = 0;
		double minReprojErr = distErr[0];

		for ( int indexT = 1; indexT < rtsFileList.size(); indexT ++ )
		{
			if ( minReprojErr > distErr[indexT] )
			{
				minReprojErr = distErr[indexT];
				minIndex = indexT;
			}
		}

		minReprojErr = reProjErr[minIndex];
		...
		string RTSFile = outDir + "/dendogram/cluster" + clusterMergeList[cIndex].newCluster + "/" + rtsFileList[minIndex];
		ifstream myfile1;
		myfile1.open(RTSFile.c_str());
		...
		cout << "Pt 1 " << ptAttrList1.size() << " PT 2: " << ptAttrList2.size() << endl;
		...
		for ( int index2 = 0; index2 < ptAttrList2.size(); index2 ++ )
		{
			...
			ptAttrList2[index2]->X = pt2[0];
			ptAttrList2[index2]->Y = pt2[1];
			ptAttrList2[index2]->Z = pt2[2];
		}	
		...
		if ( minReprojErr > 10.0 )
		{
			bundlerCmd = "../bin/optimizer_all " + newFolderName + "/ 0 50 16 32 0.5 0";
		}else
		{
			bundlerCmd = "../bin/optimizer_all " + newFolderName + "/ -1 5 16 32 0.5 0";
		}
