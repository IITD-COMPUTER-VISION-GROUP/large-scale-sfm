...
static  void ComputeRigidTransformWithR(PointsType& src, PointsType& dst, double &scale, Eigen::Matrix3d &rot, Eigen::Vector3d &tran)
{
	//Determine the scale
	...
	//Compute R & T
	bestRT = ComputeRTWithRot(tmpSrc, tmpDst);*/
...
int main(int argc, char *argv[])
{
	...
	for ( int cIndex = StIndexG; cIndex < clusterMergeList.size(); cIndex ++)
	{
		...
		string tmpPly = outDir + "/dendogram/cluster" + clusterMergeList[cIndex].newCluster + "/tmp.ply";
				
		/*FILE *fpTmp = fopen(tmpPly.c_str(), "w");

		fprintf(fpTmp,"      \n");
		fprintf(fpTmp,"                              \n");
		fprintf(fpTmp,"                              \n");
		fprintf(fpTmp,"property float x\n");
		fprintf(fpTmp,"property float y\n");
		fprintf(fpTmp,"property float z\n");
		fprintf(fpTmp,"property uchar red\n");
		fprintf(fpTmp,"property uchar green\n");
		fprintf(fpTmp,"property uchar blue\n");
		fprintf(fpTmp,"end_header\n");*/
		...
		for ( int index1 = 0; index1 < ptAttrList1.size(); index1 ++ )
		{
			for ( int index2 = 0; index2 < ptAttrList2.size(); index2 ++ )
			{
				if ( ptAttrList1[index1]->pointId == ptAttrList2[index2]->pointId )
				{
					...
					//fprintf(fpTmp,"%.10lf %.10lf %.10lf 0 255 0\n", ptAttrList1[index1]->X, ptAttrList1[index1]->Y, ptAttrList1[index1]->Z);

					//fprintf(fpTmp,"%.10lf %.10lf %.10lf 255 0 0 \n", ptAttrList2[index2]->X, ptAttrList2[index2]->Y, ptAttrList2[index2]->Z);
		...
		ComputeRigidTransformWithR(secondList, firstList, scale, RT.first, RT.second);
		//RT = ComputeRTWithRot(secondList, firstList);

#if 0
		for ( int index2 = 0; index2 < secondList.size(); index2 ++ )
		{
			Eigen::Vector3d pt2 = secondList[index2];
		...
		fprintf(fpTmp,"ply\n");
		fprintf(fpTmp,"format ascii 1.0\n");
		fprintf(fpTmp,"element vertex %d", 2 * firstList.size());

		fclose(fpTmp);
#endif

		/*scale = 0.628166;
		RT.first << -0.5492533082, 0.1343391482, 0.8247871220, 0.0623447603, 0.9908305486, -0.1198664037, -0.8333270272, -0.0144158634, -0.5525922987;
		RT.second << 0.00998985, -0.00194556, -0.0448428;*/
