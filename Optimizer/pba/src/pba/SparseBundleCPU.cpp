...
{
	fprintf(fp, "%.10f ", c[tmp]);
}
fprintf(fp, "\n");

for(int tmp = 0; tmp < 3; tmp ++)
{
	fprintf(fp, "%.10f ", m[tmp]);
}
fprintf(fp, "\n");
...
FILE *fp = fopen("vsfm.config", "a");
fprintf(fp, "_num_imgpt %d __focal_scaling %.10f mse_convert_ratio %.10f\n", _num_imgpt, __focal_scaling, mse_convert_ratio);
fclose(fp);
...
fp = fopen("vsfm.config", "a");
fprintf(fp, "_projection_sse %.10f error_display_ratio %.10f\n", _projection_sse, error_display_ratio);
fclose(fp);
