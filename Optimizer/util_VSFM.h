...
bool LoadNVM(ifstream& in, vector<CameraT>& camera_data, vector<Point3D>& point_data,
              vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx,
              vector<string>& names, vector<int>& ptc,vector<int>&siftID,vector<vector<pair<pair<int,int>, pair<float,float>>>> &alldata)
{
	...
	alldata.resize(npoint);
    for(int i = 0; i < npoint; ++i)
    {
        float pt[3]; int cc[3], npj;
        in  >> pt[0] >> pt[1] >> pt[2] 
            >> cc[0] >> cc[1] >> cc[2] >> npj;
			alldata[i].resize(npj);
        for(int j = 0; j < npj; ++j)
        {
            int cidx, fidx; float imx, imy;
            in >> cidx >> fidx >> imx >> imy;
			
			alldata[i][j].first.first = cidx;
			alldata[i][j].first.second = fidx;
			alldata[i][j].second.first = imx;
			alldata[i][j].second.second = imy;

			camidx.push_back(cidx);    //camera index
            ptidx.push_back(i);        //point index
			siftID.push_back(fidx);
...
void SaveNVM(const char* filename, vector<CameraT>& camera_data, vector<Point3D>& point_data,
              vector<Point3D>& measurements, vector<int>& ptidx, vector<int>& camidx, 
              vector<string>& names, vector<int>& ptc)
{
    std::cout << "Saving model to " << filename << "...\n"; 
    ofstream out(filename);
...
#if MEASUREMENT_DISTORTION
        out << cam.t[0] << ' ' << cam.t[1] << ' ' << cam.t[2] << ' '
            << cam.GetMeasurementDistortion() << " 0\n"; 
#else
        out << cam.t[0] << ' ' << cam.t[1] << ' ' << cam.t[2] << ' '
    ...
    for(size_t i = 0, j = 0; i < point_data.size(); ++i)
    {
		...
		for(; j < je; ++j)    out << camidx[j] << ' ' <<  measurements[j].xyz[0] << ' ' << measurements[j].xyz[1] << ' ' << measurements[j].xyz[2] << ' ';
